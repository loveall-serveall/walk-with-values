...
//include( plugin_dir_path( __FILE__ ) . '../../saiwalk_with_values/mongo-db-api-calls.php');
include $_SERVER['DOCUMENT_ROOT']."/lib/httpful.phar";
$GLOBALS['newuserdata'] = 'newuserdata';
...
class Profile_Builder_Form_Creator{
				...
				if ( ( 'POST' == $_SERVER['REQUEST_METHOD'] ) && ( $_POST['action'] == $this->args['form_type'] ) ){
                    ...
                    if( $this->args['form_type'] == 'register' ){
                        ...
                        switch ( $account_management_settings ){
                            case 'ec-no_aa-no':
                               $this->create_user_in_mongodb(); // invoke node api function after creating user in mysql
	...
	function wppb_save_form_values( $global_request ){
        ...
        $wwvappuser = $userdata;
        $wwvappuser['userid'] = $user_id;
         $GLOBALS['newuserdata'] = $wwvappuser;
		return $user_id;
	}
...
// Jagdeesh - Function to invoke node api layer to create the user in mongodb
    function create_user_in_mongodb() {
        $service_url = API_ROOT_URL.'/users';
         error_log('createuserinappdb api endpoint '.$service_url.' ', 3, DEFAULT_LOG);
          $wwvappresult =\Httpful\Request::post($service_url)
          ->body(json_encode($GLOBALS['newuserdata']))
          ->sendsJson()
          ->send();
        error_log('createuserinappdb user created in master db', 3, DEFAULT_LOG);
        return null;
    }
}
