...
//PHP REST Library to talk to MongoDB
...
function wsl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $requested_user_login, $requested_user_email )
{
	...
	if( ! $user_id || ! is_integer( $user_id ) )
	{
		$user_id = wp_insert_user( $userdata );
               ...
               //Create new user in MongoDB only if new user is successfully added to MySQL
                if( ! $user_id || ! is_integer( $user_id ) )
                {
                     create_new_user_in_mongodb($userdata);
                }
	}
...
// Jagdeesh - Function to invoke node api layer to create the user in mongodb
    ...
    function create_new_user_in_mongodb(&$userdata) {
        $service_url = API_ROOT_URL.'/users';
         error_log('create user in app db api endpoint '.$service_url.' ', 3, DEFAULT_LOG);
          $wwvappresult =\Httpful\Request::post($service_url)
          ->body(json_encode($userdata))
          ->sendsJson()
          ->send();

        error_log('create user in app db user created in master db', 3, DEFAULT_LOG);
